...
public class KlobukHandler : MonoBehaviour
{
        ...
        switch (action)
        {
            case 0:
                ...
                if (SkillEffectTarget == 1)
                {
                    ...
                    clone.GetComponent<KlobukSkillEffect>().PlayAnimation(0);
    ...
    public float DamageCalculation(float accuracy = 0, float luck = 0, float spellDamage = 0)  // kalkulowanie obrazen
    {
        ...
        if (accuracyCheck < accuracy)
        {
            if (critCheck < crit)
            {
                ...
                show = "K³obuk uderza krytycznie za ";
                return Damage;
            }
            else
            {
